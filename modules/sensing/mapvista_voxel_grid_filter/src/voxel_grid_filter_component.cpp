...
#include "mapvista_voxel_grid_filter/voxel_grid_filter_component.hpp"

namespace mapvista_voxel_grid_filter
{
...
: Node("voxel_grid_filter", options)
{
  ...
  sub_pcd_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
    ...
    std::bind(&VoxelGridFilter::pcdCallback, this, _1));
}
...
void VoxelGridFilter::pcdCallback(const sensor_msgs::msg::PointCloud2::SharedPtr pcd_msg_ptr)
{
  ...
  auto filtered_pcd_cloud_ptr = std::make_shared<pcl::PointCloud<pcl::PointXYZ>>();
...
RCLCPP_COMPONENTS_REGISTER_NODE(mapvista_voxel_grid_filter::VoxelGridFilter)
